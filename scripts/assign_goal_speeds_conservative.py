...
with ENGINE.connect() as conn:
    conn.execute(
        text(
            """
        DROP TABLE IF EXISTS typologies_joined_c;
        COMMIT;
        CREATE TABLE typologies_joined_c AS
...
with ENGINE.connect() as conn:
    conn.execute(
        text(
            """
            alter table typologies_joined_c add column if not exists bike_mix text;
            alter table typologies_joined_c add column if not exists ped_mix text;
            alter table typologies_joined_c add column if not exists modal_mix text;
            alter table typologies_joined_c add column if not exists activ_level text;
...
tbl.to_postgis("typologies_joined_c", con=ENGINE, if_exists="replace")
...
# conn.execute(
#     """
#     create table if not exists typologies_buffer5_c as
